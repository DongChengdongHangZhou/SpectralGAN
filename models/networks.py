...
def define_G(netG, init_type='normal', init_gain=0.02, gpu_ids=[]):
    net = None
    if netG == 'unet_256':
        # net = Unet(input_channel=1,output_channel=1)
        net = NestedUNet()
    else:
...
class simpleCNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(  # input shape (2, 8, 8)
            nn.Conv2d(
            ...
            nn.ReLU(inplace=False),    # activation
        ...
        self.out = nn.Linear(32 * 3 * 3, 2)   # fully connected layer, output 10 classes
